...
export class CalculatorComponent implements OnInit {
    ...
    switch (this.lastOperator) {
      case '+':
        ...
        this.apiService.add({ parameter1: this.firstOperand, parameter2: secondOperand! })
          .subscribe((res: ResultDto) => this.handleResult(expression, res.result, '+', this.firstOperand!, secondOperand!), (_: any) => this.display = 'Hata');
        break;
      case '-':
        expression = `${this.firstOperand}-${secondOperand}`;
      ...
      case '√':
        ...
        this.apiService.squareRoot({ parameter1: this.firstOperand })
          .subscribe((res: ResultDto) => this.handleResult(expression, res.result, '√', this.firstOperand!), (_: any) => this.display = 'Hata');
        break;
    }
    this.firstOperand = null;
    this.lastOperator = null;
  ...
  private handleResult(expression: string, result: number, operation: string, parameter1: number, parameter2?: number) {
